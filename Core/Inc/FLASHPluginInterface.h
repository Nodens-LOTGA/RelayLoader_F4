#ifndef FLASH_PLUGIN_INTERFACE
#define FLASH_PLUGIN_INTERFACE
...
// endOfStack points to the end of the stack area allocated by OpenOCD directly beyond the bounds of the image. It is not equal to the usual _estack that points
// to the end of RAM. If the FLASH plugin is designed to run on several different devices with different RAM sizes, it can detect the end of RAM in this function
// and return the area between the endOfStack and end of RAM as the work area.
WorkAreaInfo FLASHPlugin_FindWorkArea(void *endOfStack);

// This function simply erases 'sectorCount' sectors starting from 'firstSector'. It should return the amount of sectors successfully erased or a negative value
// to indicate an error.
int FLASHPlugin_EraseSectors(unsigned firstSector, unsigned sectorCount);

// This function is called when the plugin is to be unloaded. It should revert any changes to hardware made during initialization (e.g. disable previousl
// yenabled interrupts). Note that OpenOCD will automatically backup/restore all memory occupied by the plugin image's sections, so the plugin can use
// global/static variables to record the previous hardware state.
...
// This function performs asynchronous programming. OpenOCD will modify the WritePointer field of 'pData' each time it writes new data to the buffer and will
// expect the function to update the ReadPointer field each time it reads a chunk of data. The WritePointer/ReadPointer will only be updated in multiples of
// WriteBlockSize returned in FLASHPlugin_Probe(). The FLASH plugin framework provides a default implementation of this method in FLASHPluginCommon.cpp that
// calls FLASHPlugin_DoProgramSync() to perform the actual programming.
int FLASHPlugin_ProgramAsync(unsigned startOffset, FIFOHeader *pData, const void *pEndOfData, unsigned bytesToWrite);

// This function should be called by the plugin once it completes initialization. The contents of the function is arbitrary. OpenOCD will set a breakpoint in
// this function to detect when the initialization completes. Once the breakpoint triggers, OpenOCD will intercept the program flow and call various
// FLASHPlugin() functions.
...
// This function is called by FLASHPlugin_ProgramAsync() to perform the actual programming. It should return the amount of bytes successfully programmed or a
// negative value to indicate an error. The bytesToWrite will always be a multiple of WriteBlockSize except for the last block.
